...
void function_(int time, int index, long long size, int dst, int sock, int PORT){
    ...
    while(bytes_sent < size){
        ...
        if(size - bytes_sent < chunk_size){
                send_r = sendto(sock, glob_content, size - bytes_sent, MSG_CONFIRM, (const struct sockaddr *) &serv_addr, sizeof(serv_addr));
                if(send_r < 0){
                ...
                send_r = sendto(sock, glob_content, chunk_size, MSG_CONFIRM, (const struct sockaddr *) &serv_addr, sizeof(serv_addr));
                bytes_sent += send_r;
            }
